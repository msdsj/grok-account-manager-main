...
import { Box, ChevronDown, Eye, ExternalLink, HeartHandshake, Image, KeyRound, Languages, LayoutDashboard, LogOut, Menu, MessageSquareText, Monitor, Moon, MoreHorizontal, Play, Settings, ShieldCheck, Sparkles, Sun, Users, Video } from "lucide-react";
...
const navigation = [
  ...
  { href: "/register", label: "nav.registerTask", icon: Play },
        ...
        <aside className="fixed inset-y-0 left-0 z-30 hidden h-screen w-[288px] flex-col overflow-hidden bg-sidebar px-4 py-6 lg:flex">
          <div className="flex h-7 shrink-0 items-center justify-between px-2.5">
            ...
            <Button variant="ghost" size="icon" className="size-7 text-muted-foreground [&_svg]:size-[15px]" asChild>
              <a href="https://github.com/LXXYSLF/grok-account-manager-main" target="_blank" rel="noreferrer" aria-label="GitHub">
                <GitHubMark />
              </a>
            </Button>
          </div>
      ...
      <Dialog open={attributionOpen} onOpenChange={setAttributionOpen}>
        <DialogContent className="max-w-[560px]">
          <DialogHeader>
            ...
            <DialogTitle>{t("community.noticeTitle")}</DialogTitle>
            <DialogDescription>{t("community.noticeDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 rounded-lg border border-border/70 bg-secondary/35 p-4 text-sm">
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">{t("community.noticeUpstream")}</span>
              <a className="inline-flex items-center gap-1.5 font-medium hover:underline" href="https://github.com/chenyme/grok2api" target="_blank" rel="noreferrer">
                chenyme/grok2api <ExternalLink className="size-3.5" />
              </a>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">{t("community.noticeCustom")}</span>
              <a className="inline-flex items-center gap-1.5 font-medium hover:underline" href="https://github.com/LXXYSLF/grok-account-manager-main" target="_blank" rel="noreferrer">
                Grok 2api <ExternalLink className="size-3.5" />
              </a>
            </div>
          </div>
          <DialogFooter>
