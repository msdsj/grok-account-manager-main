...
class RegistrationJobManager:
    ...
    def start(self, total: int, concurrency: int, oauth_exchange: bool) -> dict:
        ...
        max_concurrency = _safe_int(
            os.environ.get("AI_SIGNUPER_MAX_CONCURRENCY"),
...
def refresh_account_quota(account_id: str) -> dict:
    ...
    from .grok_api import fetch_complete_credential
...
class WebHandler(BaseHTTPRequestHandler):
    server_version = "AiSignuperWeb/0.1"
...
def run(host: str = "127.0.0.1", port: int = 8765) -> None:
    ...
    print(f"[*] AI Signuper API 已启动: http://{host}:{port}")
